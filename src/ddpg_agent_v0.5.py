...
from tf_agents.agents import DdpgAgent, PPOAgent, Td3Agent
...
class DroneEnvironment(py_environment.PyEnvironment):
  ...
  def reset_pose(self):
    ...
    self.initial_pose = airsim.Pose(position_val=airsim.Vector3r(0.0, 0.0, -100.0), orientation_val=airsim.Quaternionr(0.0, 0.0, 0.0, 1.0))
    #self.client.simSetObjectPose(object_name="SimpleFlight", pose=self.initial_pose, teleport=False)
    self.client.simSetVehiclePose(pose=self.initial_pose, ignore_collision=False, vehicle_name="SimpleFlight")
  ...
  def reward_function(self, pose):
    reward = -abs(pose.position.z_val-self.initial_pose.position.z_val)/10 -abs(pose.position.x_val-self.initial_pose.position.x_val)/10 -abs(pose.position.y_val-self.initial_pose.position.y_val)/10 # Manhattan distance from initial pose
    return reward
...
# REINFORCE code - not usable
#agent = reinforce_agent.ReinforceAgent(tf_env.time_step_spec(),
#                        tf_env.action_spec(),
#                        actor_network=actor_net,
...
# SAC agent code
#agent = sac_agent.SacAgent(tf_env.time_step_spec(),
#                            tf_env.action_spec(),
#                            actor_network=actor_net,
#                            critic_network=critic_net,
