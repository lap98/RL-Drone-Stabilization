...
class DroneEnvironmentWind(py_environment.PyEnvironment):
  ...
  def reward_function(self, pose):
    # loss = abs(pose.position.z_val-self.initial_pose.position.z_val) + abs(pose.position.x_val-self.initial_pose.position.x_val) + abs(pose.position.y_val-self.initial_pose.position.y_val) # Manhattan distance from initial pose
